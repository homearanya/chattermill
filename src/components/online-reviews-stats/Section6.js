import React, { Component } from "react"
import * as handleShare from "./util.js"
import { CopyToClipboard } from "react-copy-to-clipboard"
import { GatsbyImage } from "gatsby-plugin-image"
import { useStaticQuery, graphql } from "gatsby"

const Section6 = () => {
  const { section6Header, statImg7 } = useStaticQuery(
    graphql`
      {
        section6Header: file(
          relativePath: {
            eq: "online-reviews-stats/illustration-header-section-06.png"
          }
        ) {
          childImageSharp {
            gatsbyImageData(
              width: 580
              layout: CONSTRAINED
              placeholder: BLURRED
              formats: [AUTO, WEBP]
            )
          }
        }
        statImg7: file(
          relativePath: { eq: "online-reviews-stats/illustration-stat-07.png" }
        ) {
          childImageSharp {
            gatsbyImageData(
              width: 300
              layout: CONSTRAINED
              placeholder: BLURRED
              formats: [AUTO, WEBP]
            )
          }
        }
      }
    `
  )
  const getImage = img => img?.childImageSharp?.gatsbyImageData
  return (
    <section id="section6" className="stat-section">
      <div className="section-wrapper">
        <div className="section-header">
          <div className="section-header-text-container">
            <h2 className="section-header-title">
              Online Reviews and COVID-19
            </h2>
          </div>
          <div className="section-header-img-container">
            <GatsbyImage
              image={getImage(section6Header)}
              alt="A frowning germ next to hand sanitizer"
            />
            <div className="header-circle"></div>
          </div>
        </div>
        <ul className="stats-grid sg6">
          <li className="stat-block sg6-1">
            <p className="stat-block-copy">
              41% of{" "}
              <span className="bold-class">
                Americans are more inclined to support small businesses
              </span>{" "}
              now than they were prior to COVID-19.
            </p>
            <div className="bottom-info-container">
              <a
                className="stat-block-source"
                href="https://learn.podium.com/eb-sor-2020"
                target="_blank"
              >
                Podium
              </a>
              <div className="stat-share-container">
                <a
                  className="stat-twitter-icon"
                  onClick={handleShare.shareTweet}
                >
                  <svg
                    width="20px"
                    height="18px"
                    viewBox="0 0 20 18"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <title>icon-twitter</title>
                    <g
                      id="Page-1"
                      stroke="none"
                      strokeWidth="1"
                      fill="none"
                      fillRule="evenodd"
                    >
                      <g
                        id="Customer-Experience-Statistics---V1"
                        transform="translate(-420.000000, -1084.000000)"
                        fill="#10C2ED"
                        fillRule="nonzero"
                      >
                        <g
                          id="Group-26"
                          transform="translate(250.000000, 448.000000)"
                        >
                          <g
                            id="Group-24"
                            transform="translate(0.000000, 419.500000)"
                          >
                            <g
                              id="Group"
                              transform="translate(30.000000, 212.000000)"
                            >
                              <path
                                d="M146.294388,21.6218323 C153.832479,21.6218323 157.956805,15.3781796 157.956805,9.9594532 C157.956805,9.78180317 157.956805,9.60407685 157.944102,9.42642682 C158.743585,8.84266509 159.441596,8.11932392 159.999876,7.30717681 C159.26387,7.62440628 158.464388,7.85286795 157.6395,7.94167389 C158.489717,7.43409149 159.136917,6.63460914 159.44152,5.68280629 C158.654664,6.15235626 157.766376,6.49499155 156.840017,6.67264158 C156.091232,5.87315923 155.025294,5.37824159 153.845067,5.37824159 C151.573535,5.37824159 149.746109,7.21833272 149.746109,9.47720032 C149.746109,9.79446794 149.784179,10.1116974 149.847618,10.4162621 C146.44667,10.2385739 143.41365,8.61424157 141.395909,6.12695044 C141.040532,6.73607985 140.837553,7.43405334 140.837553,8.1954651 C140.837553,9.61677976 141.560856,10.8731092 142.664903,11.6091532 C141.992373,11.5838237 141.3578,11.3934708 140.812147,11.0889061 L140.812147,11.1396414 C140.812147,13.1320149 142.220797,14.781715 144.098959,15.162459 C143.756362,15.2512649 143.38832,15.3020384 143.020318,15.3020384 C142.753785,15.3020384 142.5,15.2766708 142.246165,15.2386002 C142.766488,16.8629707 144.276647,18.0431589 146.078667,18.0812676 C144.670018,19.1853147 142.906029,19.8325147 140.989835,19.8325147 C140.6472,19.8325147 140.329971,19.8198118 140,19.7817793 C141.814723,20.9492265 143.972044,21.6218323 146.294388,21.6218323 Z"
                                id="icon-twitter"
                              ></path>
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </a>
                <CopyToClipboard text="41% of Americans are more inclined to support small businesses now than they were prior to COVID-19. via Chattermill">
                  <a
                    className="stat-clipboard-icon"
                    onClick={handleShare.shareClip}
                  >
                    <svg
                      width="18px"
                      height="20px"
                      viewBox="0 0 18 20"
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <title>icon-copy</title>
                      <g
                        id="Page-1"
                        stroke="none"
                        strokeWidth="1"
                        fill="none"
                        fillRule="evenodd"
                      >
                        <g
                          id="Customer-Experience-Statistics---V1"
                          transform="translate(-390.000000, -1083.000000)"
                          fill="#10C2ED"
                          fillRule="nonzero"
                        >
                          <g
                            id="Group-26"
                            transform="translate(250.000000, 448.000000)"
                          >
                            <g
                              id="Group-24"
                              transform="translate(0.000000, 419.500000)"
                            >
                              <g
                                id="Group"
                                transform="translate(30.000000, 212.000000)"
                              >
                                <path
                                  d="M120.624975,23.499994 C121.66051,23.499994 122.49997,22.6605338 122.49997,21.6249985 L122.49997,19.750003 L125.624963,19.750003 C126.660498,19.750003 127.499958,18.9105427 127.499958,17.8750075 L127.499958,7.40166664 C127.499958,6.97297198 127.253911,6.37898689 126.950796,6.07587176 L124.924128,4.04920423 C124.621013,3.74608911 124.027028,3.50004172 123.598333,3.50004172 L116.874984,3.50004172 C115.839448,3.50004172 114.999988,4.339502 114.999988,5.37503725 L114.999988,7.25003278 L111.874996,7.25003278 C110.83946,7.25003278 110,8.08949306 110,9.12502831 L110,21.6249985 C110,22.6605338 110.83946,23.499994 111.874996,23.499994 L120.624975,23.499994 Z M125.390588,17.8750075 L117.109358,17.8750075 C116.980002,17.8750075 116.874984,17.7699891 116.874984,17.640633 L116.874984,5.60941169 C116.874984,5.48005562 116.980002,5.37503725 117.109358,5.37503725 L121.249973,5.37503725 L121.249973,8.81252906 C121.249973,9.33029668 121.669703,9.75002682 122.187471,9.75002682 L125.624963,9.75002682 L125.624963,17.640633 C125.624963,17.7699891 125.519944,17.8750075 125.390588,17.8750075 Z M125.624969,7.87503725 L123.124969,7.87503725 L123.124969,5.37503725 L123.501211,5.37503725 C123.563353,5.37503725 123.622976,5.39971829 123.66696,5.44366349 L125.556337,7.3330404 C125.58159,7.35829363 125.603673,7.39970419 125.615589,7.44038572 L125.624969,7.4987886 L125.624969,7.87503725 Z M120.3906,21.6249985 L112.10937,21.6249985 C111.980014,21.6249985 111.874996,21.5199801 111.874996,21.3906241 L111.874996,9.35940275 C111.874996,9.23004668 111.980014,9.12502831 112.10937,9.12502831 L114.999988,9.12502831 L114.999988,17.8750075 C114.999988,18.9105427 115.839448,19.750003 116.874984,19.750003 L120.624975,19.750003 L120.624975,21.3906241 C120.624975,21.5199801 120.519956,21.6249985 120.3906,21.6249985 Z"
                                  id="icon-copy"
                                ></path>
                              </g>
                            </g>
                          </g>
                        </g>
                      </g>
                    </svg>
                  </a>
                </CopyToClipboard>
              </div>
            </div>
          </li>

          <li className="stat-block alt-light-blue sg6-2">
            <p className="stat-block-copy">
              39% of Americans{" "}
              <span className="bold-class">
                want to see businesses enforce mask requirements
              </span>
              .
            </p>
            <div className="bottom-info-container">
              <a
                className="stat-block-source"
                href="https://learn.podium.com/eb-sor-2020"
                target="_blank"
              >
                Podium
              </a>
              <div className="stat-share-container">
                <a
                  className="stat-twitter-icon"
                  onClick={handleShare.shareTweet}
                >
                  <svg
                    width="20px"
                    height="18px"
                    viewBox="0 0 20 18"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <title>icon-twitter</title>
                    <g
                      id="Page-1"
                      stroke="none"
                      strokeWidth="1"
                      fill="none"
                      fillRule="evenodd"
                    >
                      <g
                        id="Customer-Experience-Statistics---V1"
                        transform="translate(-420.000000, -1084.000000)"
                        fill="#10C2ED"
                        fillRule="nonzero"
                      >
                        <g
                          id="Group-26"
                          transform="translate(250.000000, 448.000000)"
                        >
                          <g
                            id="Group-24"
                            transform="translate(0.000000, 419.500000)"
                          >
                            <g
                              id="Group"
                              transform="translate(30.000000, 212.000000)"
                            >
                              <path
                                d="M146.294388,21.6218323 C153.832479,21.6218323 157.956805,15.3781796 157.956805,9.9594532 C157.956805,9.78180317 157.956805,9.60407685 157.944102,9.42642682 C158.743585,8.84266509 159.441596,8.11932392 159.999876,7.30717681 C159.26387,7.62440628 158.464388,7.85286795 157.6395,7.94167389 C158.489717,7.43409149 159.136917,6.63460914 159.44152,5.68280629 C158.654664,6.15235626 157.766376,6.49499155 156.840017,6.67264158 C156.091232,5.87315923 155.025294,5.37824159 153.845067,5.37824159 C151.573535,5.37824159 149.746109,7.21833272 149.746109,9.47720032 C149.746109,9.79446794 149.784179,10.1116974 149.847618,10.4162621 C146.44667,10.2385739 143.41365,8.61424157 141.395909,6.12695044 C141.040532,6.73607985 140.837553,7.43405334 140.837553,8.1954651 C140.837553,9.61677976 141.560856,10.8731092 142.664903,11.6091532 C141.992373,11.5838237 141.3578,11.3934708 140.812147,11.0889061 L140.812147,11.1396414 C140.812147,13.1320149 142.220797,14.781715 144.098959,15.162459 C143.756362,15.2512649 143.38832,15.3020384 143.020318,15.3020384 C142.753785,15.3020384 142.5,15.2766708 142.246165,15.2386002 C142.766488,16.8629707 144.276647,18.0431589 146.078667,18.0812676 C144.670018,19.1853147 142.906029,19.8325147 140.989835,19.8325147 C140.6472,19.8325147 140.329971,19.8198118 140,19.7817793 C141.814723,20.9492265 143.972044,21.6218323 146.294388,21.6218323 Z"
                                id="icon-twitter"
                              ></path>
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </a>
                <CopyToClipboard text="39% of Americans want to see businesses enforce mask requirements. via Chattermill">
                  <a
                    className="stat-clipboard-icon"
                    onClick={handleShare.shareClip}
                  >
                    <svg
                      width="18px"
                      height="20px"
                      viewBox="0 0 18 20"
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <title>icon-copy</title>
                      <g
                        id="Page-1"
                        stroke="none"
                        strokeWidth="1"
                        fill="none"
                        fillRule="evenodd"
                      >
                        <g
                          id="Customer-Experience-Statistics---V1"
                          transform="translate(-390.000000, -1083.000000)"
                          fill="#10C2ED"
                          fillRule="nonzero"
                        >
                          <g
                            id="Group-26"
                            transform="translate(250.000000, 448.000000)"
                          >
                            <g
                              id="Group-24"
                              transform="translate(0.000000, 419.500000)"
                            >
                              <g
                                id="Group"
                                transform="translate(30.000000, 212.000000)"
                              >
                                <path
                                  d="M120.624975,23.499994 C121.66051,23.499994 122.49997,22.6605338 122.49997,21.6249985 L122.49997,19.750003 L125.624963,19.750003 C126.660498,19.750003 127.499958,18.9105427 127.499958,17.8750075 L127.499958,7.40166664 C127.499958,6.97297198 127.253911,6.37898689 126.950796,6.07587176 L124.924128,4.04920423 C124.621013,3.74608911 124.027028,3.50004172 123.598333,3.50004172 L116.874984,3.50004172 C115.839448,3.50004172 114.999988,4.339502 114.999988,5.37503725 L114.999988,7.25003278 L111.874996,7.25003278 C110.83946,7.25003278 110,8.08949306 110,9.12502831 L110,21.6249985 C110,22.6605338 110.83946,23.499994 111.874996,23.499994 L120.624975,23.499994 Z M125.390588,17.8750075 L117.109358,17.8750075 C116.980002,17.8750075 116.874984,17.7699891 116.874984,17.640633 L116.874984,5.60941169 C116.874984,5.48005562 116.980002,5.37503725 117.109358,5.37503725 L121.249973,5.37503725 L121.249973,8.81252906 C121.249973,9.33029668 121.669703,9.75002682 122.187471,9.75002682 L125.624963,9.75002682 L125.624963,17.640633 C125.624963,17.7699891 125.519944,17.8750075 125.390588,17.8750075 Z M125.624969,7.87503725 L123.124969,7.87503725 L123.124969,5.37503725 L123.501211,5.37503725 C123.563353,5.37503725 123.622976,5.39971829 123.66696,5.44366349 L125.556337,7.3330404 C125.58159,7.35829363 125.603673,7.39970419 125.615589,7.44038572 L125.624969,7.4987886 L125.624969,7.87503725 Z M120.3906,21.6249985 L112.10937,21.6249985 C111.980014,21.6249985 111.874996,21.5199801 111.874996,21.3906241 L111.874996,9.35940275 C111.874996,9.23004668 111.980014,9.12502831 112.10937,9.12502831 L114.999988,9.12502831 L114.999988,17.8750075 C114.999988,18.9105427 115.839448,19.750003 116.874984,19.750003 L120.624975,19.750003 L120.624975,21.3906241 C120.624975,21.5199801 120.519956,21.6249985 120.3906,21.6249985 Z"
                                  id="icon-copy"
                                ></path>
                              </g>
                            </g>
                          </g>
                        </g>
                      </g>
                    </svg>
                  </a>
                </CopyToClipboard>
              </div>
            </div>
          </li>

          <li className="stat-block sg6-3">
            <p className="stat-block-copy">
              <span className="bold-class">
                27% of people think social-distancing requirements are important
              </span>{" "}
              — nearly equal to the importance of store appearance (28%).
            </p>
            <div className="stat-img-wrapper">
              <GatsbyImage
                image={getImage(statImg7)}
                alt="A tape measurer with 6ft written next to it"
              />
              <div className="stat-block-img-bg"></div>
            </div>
            <div className="bottom-info-container">
              <a
                className="stat-block-source"
                href="https://learn.podium.com/eb-sor-2020"
                target="_blank"
              >
                Podium
              </a>
              <div className="stat-share-container">
                <a
                  className="stat-twitter-icon"
                  onClick={handleShare.shareTweet}
                >
                  <svg
                    width="20px"
                    height="18px"
                    viewBox="0 0 20 18"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <title>icon-twitter</title>
                    <g
                      id="Page-1"
                      stroke="none"
                      strokeWidth="1"
                      fill="none"
                      fillRule="evenodd"
                    >
                      <g
                        id="Customer-Experience-Statistics---V1"
                        transform="translate(-420.000000, -1084.000000)"
                        fill="#10C2ED"
                        fillRule="nonzero"
                      >
                        <g
                          id="Group-26"
                          transform="translate(250.000000, 448.000000)"
                        >
                          <g
                            id="Group-24"
                            transform="translate(0.000000, 419.500000)"
                          >
                            <g
                              id="Group"
                              transform="translate(30.000000, 212.000000)"
                            >
                              <path
                                d="M146.294388,21.6218323 C153.832479,21.6218323 157.956805,15.3781796 157.956805,9.9594532 C157.956805,9.78180317 157.956805,9.60407685 157.944102,9.42642682 C158.743585,8.84266509 159.441596,8.11932392 159.999876,7.30717681 C159.26387,7.62440628 158.464388,7.85286795 157.6395,7.94167389 C158.489717,7.43409149 159.136917,6.63460914 159.44152,5.68280629 C158.654664,6.15235626 157.766376,6.49499155 156.840017,6.67264158 C156.091232,5.87315923 155.025294,5.37824159 153.845067,5.37824159 C151.573535,5.37824159 149.746109,7.21833272 149.746109,9.47720032 C149.746109,9.79446794 149.784179,10.1116974 149.847618,10.4162621 C146.44667,10.2385739 143.41365,8.61424157 141.395909,6.12695044 C141.040532,6.73607985 140.837553,7.43405334 140.837553,8.1954651 C140.837553,9.61677976 141.560856,10.8731092 142.664903,11.6091532 C141.992373,11.5838237 141.3578,11.3934708 140.812147,11.0889061 L140.812147,11.1396414 C140.812147,13.1320149 142.220797,14.781715 144.098959,15.162459 C143.756362,15.2512649 143.38832,15.3020384 143.020318,15.3020384 C142.753785,15.3020384 142.5,15.2766708 142.246165,15.2386002 C142.766488,16.8629707 144.276647,18.0431589 146.078667,18.0812676 C144.670018,19.1853147 142.906029,19.8325147 140.989835,19.8325147 C140.6472,19.8325147 140.329971,19.8198118 140,19.7817793 C141.814723,20.9492265 143.972044,21.6218323 146.294388,21.6218323 Z"
                                id="icon-twitter"
                              ></path>
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </a>
                <CopyToClipboard text="27% of people think social-distancing requirements are important — nearly equal to the importance of store appearance (28%). via Chattermill">
                  <a
                    className="stat-clipboard-icon"
                    onClick={handleShare.shareClip}
                  >
                    <svg
                      width="18px"
                      height="20px"
                      viewBox="0 0 18 20"
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <title>icon-copy</title>
                      <g
                        id="Page-1"
                        stroke="none"
                        strokeWidth="1"
                        fill="none"
                        fillRule="evenodd"
                      >
                        <g
                          id="Customer-Experience-Statistics---V1"
                          transform="translate(-390.000000, -1083.000000)"
                          fill="#10C2ED"
                          fillRule="nonzero"
                        >
                          <g
                            id="Group-26"
                            transform="translate(250.000000, 448.000000)"
                          >
                            <g
                              id="Group-24"
                              transform="translate(0.000000, 419.500000)"
                            >
                              <g
                                id="Group"
                                transform="translate(30.000000, 212.000000)"
                              >
                                <path
                                  d="M120.624975,23.499994 C121.66051,23.499994 122.49997,22.6605338 122.49997,21.6249985 L122.49997,19.750003 L125.624963,19.750003 C126.660498,19.750003 127.499958,18.9105427 127.499958,17.8750075 L127.499958,7.40166664 C127.499958,6.97297198 127.253911,6.37898689 126.950796,6.07587176 L124.924128,4.04920423 C124.621013,3.74608911 124.027028,3.50004172 123.598333,3.50004172 L116.874984,3.50004172 C115.839448,3.50004172 114.999988,4.339502 114.999988,5.37503725 L114.999988,7.25003278 L111.874996,7.25003278 C110.83946,7.25003278 110,8.08949306 110,9.12502831 L110,21.6249985 C110,22.6605338 110.83946,23.499994 111.874996,23.499994 L120.624975,23.499994 Z M125.390588,17.8750075 L117.109358,17.8750075 C116.980002,17.8750075 116.874984,17.7699891 116.874984,17.640633 L116.874984,5.60941169 C116.874984,5.48005562 116.980002,5.37503725 117.109358,5.37503725 L121.249973,5.37503725 L121.249973,8.81252906 C121.249973,9.33029668 121.669703,9.75002682 122.187471,9.75002682 L125.624963,9.75002682 L125.624963,17.640633 C125.624963,17.7699891 125.519944,17.8750075 125.390588,17.8750075 Z M125.624969,7.87503725 L123.124969,7.87503725 L123.124969,5.37503725 L123.501211,5.37503725 C123.563353,5.37503725 123.622976,5.39971829 123.66696,5.44366349 L125.556337,7.3330404 C125.58159,7.35829363 125.603673,7.39970419 125.615589,7.44038572 L125.624969,7.4987886 L125.624969,7.87503725 Z M120.3906,21.6249985 L112.10937,21.6249985 C111.980014,21.6249985 111.874996,21.5199801 111.874996,21.3906241 L111.874996,9.35940275 C111.874996,9.23004668 111.980014,9.12502831 112.10937,9.12502831 L114.999988,9.12502831 L114.999988,17.8750075 C114.999988,18.9105427 115.839448,19.750003 116.874984,19.750003 L120.624975,19.750003 L120.624975,21.3906241 C120.624975,21.5199801 120.519956,21.6249985 120.3906,21.6249985 Z"
                                  id="icon-copy"
                                ></path>
                              </g>
                            </g>
                          </g>
                        </g>
                      </g>
                    </svg>
                  </a>
                </CopyToClipboard>
              </div>
            </div>
          </li>

          <li className="stat-block sg6-4">
            <p className="stat-block-copy">
              Half of shoppers are reading reviews to{" "}
              <span className="bold-class">
                make sure local businesses are following safety procedures
              </span>
              .
            </p>
            <div className="bottom-info-container">
              <a
                className="stat-block-source"
                href="https://learn.podium.com/eb-sor-2020"
                target="_blank"
              >
                Podium
              </a>
              <div className="stat-share-container">
                <a
                  className="stat-twitter-icon"
                  onClick={handleShare.shareTweet}
                >
                  <svg
                    width="20px"
                    height="18px"
                    viewBox="0 0 20 18"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <title>icon-twitter</title>
                    <g
                      id="Page-1"
                      stroke="none"
                      strokeWidth="1"
                      fill="none"
                      fillRule="evenodd"
                    >
                      <g
                        id="Customer-Experience-Statistics---V1"
                        transform="translate(-420.000000, -1084.000000)"
                        fill="#10C2ED"
                        fillRule="nonzero"
                      >
                        <g
                          id="Group-26"
                          transform="translate(250.000000, 448.000000)"
                        >
                          <g
                            id="Group-24"
                            transform="translate(0.000000, 419.500000)"
                          >
                            <g
                              id="Group"
                              transform="translate(30.000000, 212.000000)"
                            >
                              <path
                                d="M146.294388,21.6218323 C153.832479,21.6218323 157.956805,15.3781796 157.956805,9.9594532 C157.956805,9.78180317 157.956805,9.60407685 157.944102,9.42642682 C158.743585,8.84266509 159.441596,8.11932392 159.999876,7.30717681 C159.26387,7.62440628 158.464388,7.85286795 157.6395,7.94167389 C158.489717,7.43409149 159.136917,6.63460914 159.44152,5.68280629 C158.654664,6.15235626 157.766376,6.49499155 156.840017,6.67264158 C156.091232,5.87315923 155.025294,5.37824159 153.845067,5.37824159 C151.573535,5.37824159 149.746109,7.21833272 149.746109,9.47720032 C149.746109,9.79446794 149.784179,10.1116974 149.847618,10.4162621 C146.44667,10.2385739 143.41365,8.61424157 141.395909,6.12695044 C141.040532,6.73607985 140.837553,7.43405334 140.837553,8.1954651 C140.837553,9.61677976 141.560856,10.8731092 142.664903,11.6091532 C141.992373,11.5838237 141.3578,11.3934708 140.812147,11.0889061 L140.812147,11.1396414 C140.812147,13.1320149 142.220797,14.781715 144.098959,15.162459 C143.756362,15.2512649 143.38832,15.3020384 143.020318,15.3020384 C142.753785,15.3020384 142.5,15.2766708 142.246165,15.2386002 C142.766488,16.8629707 144.276647,18.0431589 146.078667,18.0812676 C144.670018,19.1853147 142.906029,19.8325147 140.989835,19.8325147 C140.6472,19.8325147 140.329971,19.8198118 140,19.7817793 C141.814723,20.9492265 143.972044,21.6218323 146.294388,21.6218323 Z"
                                id="icon-twitter"
                              ></path>
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </a>
                <CopyToClipboard text="Half of shoppers are reading reviews to make sure local businesses are following safety procedures. via Chattermill">
                  <a
                    className="stat-clipboard-icon"
                    onClick={handleShare.shareClip}
                  >
                    <svg
                      width="18px"
                      height="20px"
                      viewBox="0 0 18 20"
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <title>icon-copy</title>
                      <g
                        id="Page-1"
                        stroke="none"
                        strokeWidth="1"
                        fill="none"
                        fillRule="evenodd"
                      >
                        <g
                          id="Customer-Experience-Statistics---V1"
                          transform="translate(-390.000000, -1083.000000)"
                          fill="#10C2ED"
                          fillRule="nonzero"
                        >
                          <g
                            id="Group-26"
                            transform="translate(250.000000, 448.000000)"
                          >
                            <g
                              id="Group-24"
                              transform="translate(0.000000, 419.500000)"
                            >
                              <g
                                id="Group"
                                transform="translate(30.000000, 212.000000)"
                              >
                                <path
                                  d="M120.624975,23.499994 C121.66051,23.499994 122.49997,22.6605338 122.49997,21.6249985 L122.49997,19.750003 L125.624963,19.750003 C126.660498,19.750003 127.499958,18.9105427 127.499958,17.8750075 L127.499958,7.40166664 C127.499958,6.97297198 127.253911,6.37898689 126.950796,6.07587176 L124.924128,4.04920423 C124.621013,3.74608911 124.027028,3.50004172 123.598333,3.50004172 L116.874984,3.50004172 C115.839448,3.50004172 114.999988,4.339502 114.999988,5.37503725 L114.999988,7.25003278 L111.874996,7.25003278 C110.83946,7.25003278 110,8.08949306 110,9.12502831 L110,21.6249985 C110,22.6605338 110.83946,23.499994 111.874996,23.499994 L120.624975,23.499994 Z M125.390588,17.8750075 L117.109358,17.8750075 C116.980002,17.8750075 116.874984,17.7699891 116.874984,17.640633 L116.874984,5.60941169 C116.874984,5.48005562 116.980002,5.37503725 117.109358,5.37503725 L121.249973,5.37503725 L121.249973,8.81252906 C121.249973,9.33029668 121.669703,9.75002682 122.187471,9.75002682 L125.624963,9.75002682 L125.624963,17.640633 C125.624963,17.7699891 125.519944,17.8750075 125.390588,17.8750075 Z M125.624969,7.87503725 L123.124969,7.87503725 L123.124969,5.37503725 L123.501211,5.37503725 C123.563353,5.37503725 123.622976,5.39971829 123.66696,5.44366349 L125.556337,7.3330404 C125.58159,7.35829363 125.603673,7.39970419 125.615589,7.44038572 L125.624969,7.4987886 L125.624969,7.87503725 Z M120.3906,21.6249985 L112.10937,21.6249985 C111.980014,21.6249985 111.874996,21.5199801 111.874996,21.3906241 L111.874996,9.35940275 C111.874996,9.23004668 111.980014,9.12502831 112.10937,9.12502831 L114.999988,9.12502831 L114.999988,17.8750075 C114.999988,18.9105427 115.839448,19.750003 116.874984,19.750003 L120.624975,19.750003 L120.624975,21.3906241 C120.624975,21.5199801 120.519956,21.6249985 120.3906,21.6249985 Z"
                                  id="icon-copy"
                                ></path>
                              </g>
                            </g>
                          </g>
                        </g>
                      </g>
                    </svg>
                  </a>
                </CopyToClipboard>
              </div>
            </div>
          </li>

          <li className="stat-block sg6-5">
            <p className="stat-block-copy">
              <span className="bold-class">
                17% of people have written negative reviews
              </span>{" "}
              for companies with lacking health and safety guidelines.
            </p>
            <div className="bottom-info-container">
              <a
                className="stat-block-source"
                href="https://www.brightlocal.com/research/local-consumer-review-survey/"
                target="_blank"
              >
                BrightLocal
              </a>
              <div className="stat-share-container">
                <a
                  className="stat-twitter-icon"
                  onClick={handleShare.shareTweet}
                >
                  <svg
                    width="20px"
                    height="18px"
                    viewBox="0 0 20 18"
                    version="1.1"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <title>icon-twitter</title>
                    <g
                      id="Page-1"
                      stroke="none"
                      strokeWidth="1"
                      fill="none"
                      fillRule="evenodd"
                    >
                      <g
                        id="Customer-Experience-Statistics---V1"
                        transform="translate(-420.000000, -1084.000000)"
                        fill="#10C2ED"
                        fillRule="nonzero"
                      >
                        <g
                          id="Group-26"
                          transform="translate(250.000000, 448.000000)"
                        >
                          <g
                            id="Group-24"
                            transform="translate(0.000000, 419.500000)"
                          >
                            <g
                              id="Group"
                              transform="translate(30.000000, 212.000000)"
                            >
                              <path
                                d="M146.294388,21.6218323 C153.832479,21.6218323 157.956805,15.3781796 157.956805,9.9594532 C157.956805,9.78180317 157.956805,9.60407685 157.944102,9.42642682 C158.743585,8.84266509 159.441596,8.11932392 159.999876,7.30717681 C159.26387,7.62440628 158.464388,7.85286795 157.6395,7.94167389 C158.489717,7.43409149 159.136917,6.63460914 159.44152,5.68280629 C158.654664,6.15235626 157.766376,6.49499155 156.840017,6.67264158 C156.091232,5.87315923 155.025294,5.37824159 153.845067,5.37824159 C151.573535,5.37824159 149.746109,7.21833272 149.746109,9.47720032 C149.746109,9.79446794 149.784179,10.1116974 149.847618,10.4162621 C146.44667,10.2385739 143.41365,8.61424157 141.395909,6.12695044 C141.040532,6.73607985 140.837553,7.43405334 140.837553,8.1954651 C140.837553,9.61677976 141.560856,10.8731092 142.664903,11.6091532 C141.992373,11.5838237 141.3578,11.3934708 140.812147,11.0889061 L140.812147,11.1396414 C140.812147,13.1320149 142.220797,14.781715 144.098959,15.162459 C143.756362,15.2512649 143.38832,15.3020384 143.020318,15.3020384 C142.753785,15.3020384 142.5,15.2766708 142.246165,15.2386002 C142.766488,16.8629707 144.276647,18.0431589 146.078667,18.0812676 C144.670018,19.1853147 142.906029,19.8325147 140.989835,19.8325147 C140.6472,19.8325147 140.329971,19.8198118 140,19.7817793 C141.814723,20.9492265 143.972044,21.6218323 146.294388,21.6218323 Z"
                                id="icon-twitter"
                              ></path>
                            </g>
                          </g>
                        </g>
                      </g>
                    </g>
                  </svg>
                </a>
                <CopyToClipboard text="17% of people have written negative reviews for companies with lacking health and safety guidelines. via Chattermill">
                  <a
                    className="stat-clipboard-icon"
                    onClick={handleShare.shareClip}
                  >
                    <svg
                      width="18px"
                      height="20px"
                      viewBox="0 0 18 20"
                      version="1.1"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <title>icon-copy</title>
                      <g
                        id="Page-1"
                        stroke="none"
                        strokeWidth="1"
                        fill="none"
                        fillRule="evenodd"
                      >
                        <g
                          id="Customer-Experience-Statistics---V1"
                          transform="translate(-390.000000, -1083.000000)"
                          fill="#10C2ED"
                          fillRule="nonzero"
                        >
                          <g
                            id="Group-26"
                            transform="translate(250.000000, 448.000000)"
                          >
                            <g
                              id="Group-24"
                              transform="translate(0.000000, 419.500000)"
                            >
                              <g
                                id="Group"
                                transform="translate(30.000000, 212.000000)"
                              >
                                <path
                                  d="M120.624975,23.499994 C121.66051,23.499994 122.49997,22.6605338 122.49997,21.6249985 L122.49997,19.750003 L125.624963,19.750003 C126.660498,19.750003 127.499958,18.9105427 127.499958,17.8750075 L127.499958,7.40166664 C127.499958,6.97297198 127.253911,6.37898689 126.950796,6.07587176 L124.924128,4.04920423 C124.621013,3.74608911 124.027028,3.50004172 123.598333,3.50004172 L116.874984,3.50004172 C115.839448,3.50004172 114.999988,4.339502 114.999988,5.37503725 L114.999988,7.25003278 L111.874996,7.25003278 C110.83946,7.25003278 110,8.08949306 110,9.12502831 L110,21.6249985 C110,22.6605338 110.83946,23.499994 111.874996,23.499994 L120.624975,23.499994 Z M125.390588,17.8750075 L117.109358,17.8750075 C116.980002,17.8750075 116.874984,17.7699891 116.874984,17.640633 L116.874984,5.60941169 C116.874984,5.48005562 116.980002,5.37503725 117.109358,5.37503725 L121.249973,5.37503725 L121.249973,8.81252906 C121.249973,9.33029668 121.669703,9.75002682 122.187471,9.75002682 L125.624963,9.75002682 L125.624963,17.640633 C125.624963,17.7699891 125.519944,17.8750075 125.390588,17.8750075 Z M125.624969,7.87503725 L123.124969,7.87503725 L123.124969,5.37503725 L123.501211,5.37503725 C123.563353,5.37503725 123.622976,5.39971829 123.66696,5.44366349 L125.556337,7.3330404 C125.58159,7.35829363 125.603673,7.39970419 125.615589,7.44038572 L125.624969,7.4987886 L125.624969,7.87503725 Z M120.3906,21.6249985 L112.10937,21.6249985 C111.980014,21.6249985 111.874996,21.5199801 111.874996,21.3906241 L111.874996,9.35940275 C111.874996,9.23004668 111.980014,9.12502831 112.10937,9.12502831 L114.999988,9.12502831 L114.999988,17.8750075 C114.999988,18.9105427 115.839448,19.750003 116.874984,19.750003 L120.624975,19.750003 L120.624975,21.3906241 C120.624975,21.5199801 120.519956,21.6249985 120.3906,21.6249985 Z"
                                  id="icon-copy"
                                ></path>
                              </g>
                            </g>
                          </g>
                        </g>
                      </g>
                    </svg>
                  </a>
                </CopyToClipboard>
              </div>
            </div>
          </li>
        </ul>
      </div>
    </section>
  )
}

export default Section6
